...
// ---------------------------------------------------------------------------
// 1️⃣ ANALYTICS KERNEL (PURE FUNCTION)
// Derived strictly from trade rows. No config assumptions.
// ---------------------------------------------------------------------------
const computeAnalytics = (trades: Trade[], sessionInitialCapital: number) => {
  const totalTrades = trades.length;

  const winTrades = trades.filter(t => t.returnVal > 0).length;
  const lossTrades = trades.filter(t => t.returnVal < 0).length;

  // Gross Profit from winning trades
  const winProfit = trades
    .filter(t => t.returnVal > 0)
    .reduce((sum, t) => sum + t.returnVal, 0);

  // Deriving capital from rows if possible, else falling back to session initial
  const initialCapital = trades.length > 0
    ? trades[0].balance - trades[0].returnVal
  ...
  return {
    ...
    winProfit,      // Gross Win ($)
    accountGain,    // Net PnL ($)
    finalCapital,   // End Balance ($)
    accountGainPercent // Net PnL (%)
  };
};
...
const TradeManager: React.FC<TradeManagerProps> = ({ activeSession, sessions, onUpdateSession, onSaveSession }) => {
  ...
  // ---------------------------------------------------------------------------
  // 2️⃣ DYNAMIC TARGET SIMULATION (ROW GENERATION)
  // Generates hypothetical trade rows to determine Strategy Targets.
  // ---------------------------------------------------------------------------
  const { targetStats, strategyBullets, maxLossLimit, minCapitalRequired, isCompounding } = useMemo(() => {
    ...
    let bullets: number[] = [];
    let projectedTrades: Trade[] = [];
    let calcMinCap = 0;

    // Strategy Logic to generate Bullets and Rows
    if (compounding) {
      ...
      // Simulate: Lose (maxLosses-1) -> Win (w) on last bullet
      let simBalance = currentCap;
      let tradeIdx = 1;

      // Losses
      for (let i = 0; i < maxLosses - 1; i++) {
        const stake = bullets[i];
        const pnl = -stake;
        simBalance += pnl;
        projectedTrades.push({ no: tradeIdx++, result: 'L', tradeAmt: stake, returnVal: pnl, balance: simBalance });
      }
      // Win Streak
      let currentStake = bullets[maxLosses - 1]; 
      for (let k = 0; k < w; k++) {
        const winAmt = currentStake * payout;
        simBalance += winAmt;
        projectedTrades.push({ no: tradeIdx++, result: 'W', tradeAmt: currentStake, returnVal: winAmt, balance: simBalance });
        currentStake += winAmt;
      }
      ...
      // Simulate: Lose (maxLosses-1) -> Win 1
      let simBalance = currentCap;
      let tradeIdx = 1;
      ...
      // Final Win
      const finalStake = bullets[maxLosses - 1];
      const finalWin = finalStake * payout;
      simBalance += finalWin;
      projectedTrades.push({ no: tradeIdx++, result: 'W', tradeAmt: finalStake, returnVal: finalWin, balance: simBalance });

      if (coeffs.length > 0) calcMinCap = totalInvestedCoeff / coeffs[0];
    }

    // 3️⃣ AGGREGATE PROJECTED ROWS
    const stats = computeAnalytics(projectedTrades, currentCap);
  ...
  const currentStats = useMemo(() => {
    if (!activeSession) return computeAnalytics([], config.initialCapital);
    return computeAnalytics(activeSession.trades, activeSession.initialCapital);
  }, [activeSession, config.initialCapital]);
  ...
  const calculateTradeAmount = (prevTrades: Trade[]) => {
    const losses = prevTrades.filter(t => t.returnVal < 0).length;
    const wins = prevTrades.filter(t => t.returnVal > 0).length;
    ...
    if (isCompounding) {
      ...
      for (let i = prevTrades.length - 1; i >= 0; i--) {
        if (prevTrades[i].returnVal > 0) streak++;
        else break;
      }
      if (streak > 0) {
  ...
  const handleTradeResult = (tradeNo: number, result: 'W' | 'L') => {
    ...
    const prevBalance = currentTrades.length > 0 
      ? currentTrades[currentTrades.length - 1].balance 
      : activeSession.initialCapital;
    
    const newBalance = parseFloat((prevBalance + returnVal).toFixed(2));
    ...
    // Check completion using row counts
    const analytics = computeAnalytics(updatedTrades, activeSession.initialCapital);
  ...
  const tradeRows = useMemo(() => {
    ...
    for (let i = 1; i <= 20; i++) {
      ...
      if (existing) {
        ...
        rows.push({ no: i, result: null, tradeAmt: 0, returnVal: 0, balance: 0 });
      }
    }
    return rows;
  }, [activeSession]);
  ...
  // Daily Goals Estimation
  // We use the Target Net PnL (accountGain) to estimate sessions required.
  ...
  const sessionsRequired = targetStats.accountGain > 0 
    ? Math.max(0, Math.ceil(dailyTargetAmount / targetStats.accountGain)) 
    : 0;
    ...
    <div className="bg-[#1a1f2c] min-h-full text-slate-300 p-4 font-display">
      <div className="max-w-[1550px] mx-auto grid grid-cols-12 gap-5">
              ...
              {tradeRows.map((row, idx) => {
                ...
                return (
                  <div key={idx} className="grid grid-cols-12 border-b border-slate-800/20 items-center p-2.5 h-[34px] hover:bg-slate-800/10 transition-colors">
                    ...
                    <div className={`col-span-3 text-right text-[12px] font-bold pr-2 ${row.returnVal > 0 ? 'text-primary' : row.returnVal < 0 ? 'text-danger' : 'text-slate-700'}`}>
                      {row.result ? `${row.returnVal > 0 ? '' : ''}$${Math.abs(row.returnVal).toFixed(2)}` : ''}
                    </div>
                    <div className="col-span-3 text-right text-[12px] font-black text-white pr-4">
                      {row.result ? `$${row.balance.toLocaleString(undefined, { minimumFractionDigits: 2 })}` : ''}
                    </div>
                  </div>
                );
              })}
        ...
        <div className="col-span-12 lg:col-span-4 space-y-4">
          ...
          {/* SESSION TARGET BLOCK - FROM DYNAMIC SIMULATION */}
          <div className="bg-[#252b39] border border-slate-700 rounded overflow-hidden shadow-xl">
            <div className="bg-[#333b4d] text-center p-2.5 border-b border-slate-700">
              <span className="text-[11px] font-black uppercase text-slate-300 tracking-[0.2em]">Session Target</span>
            </div>
            <div className="p-0 text-[11px] font-bold uppercase">
              <div className="grid grid-cols-2 border-b border-slate-800/40 h-11 items-center">
                <div className="p-3 bg-slate-800/20 text-slate-400">Capital Final</div>
                <div className="p-3 text-right text-white font-black h-full flex items-center justify-end pr-4 text-sm bg-slate-900/10 tracking-tight">${targetStats.finalCapital.toLocaleString(undefined, { minimumFractionDigits: 2 })}</div>
              </div>
              <div className="grid grid-cols-2 border-b border-slate-800/40 h-11 items-center">
                <div className="p-3 bg-slate-800/20 text-slate-400">Account Gain</div>
                <div className="p-3 text-right text-white font-black h-full flex items-center justify-end pr-4 text-sm">{targetStats.accountGainPercent.toFixed(2)}%</div>
              </div>
              <div className="grid grid-cols-2 border-b border-slate-800/40 h-11 items-center">
                <div className="p-3 bg-slate-800/20 text-slate-400">Win Profit</div>
                <div className="p-3 text-right bg-primary text-slate-950 font-black h-full flex items-center justify-end pr-4 text-sm shadow-inner">${targetStats.accountGain.toFixed(2)}</div>
              </div>
              <div className="grid grid-cols-2 border-b border-slate-800/40 h-11 items-center">
                ...
                <div className="p-0 bg-slate-900/50 h-full flex items-center justify-between px-3">
                  ...
                  <span className="text-[9px] text-slate-500 font-black">10%</span>
                </div>
              </div>
              <div className="grid grid-cols-2 h-11 items-center">
                ...
                <div className="p-0 bg-slate-900/50 h-full flex items-center justify-between px-3">
                  ...
                  <span className="text-[7px] text-slate-600 font-bold uppercase leading-tight text-right pr-1">Max loss before<br/>liquidation</span>
                </div>
              </div>
            </div>
          </div>

          {/* PERFORMANCE BLOCK - FROM ACTUAL ROWS */}
          <div className="bg-[#252b39] border border-slate-700 rounded overflow-hidden shadow-xl">
             <div className="bg-[#333b4d] text-center p-2.5 border-b border-slate-700">
             ...
             <div className="p-0 text-[11px] font-bold uppercase">
                <div className="grid grid-cols-12 border-b border-slate-800/40 h-11 items-center">
                   <div className="col-span-5 p-3 bg-slate-800/20 text-slate-400 h-full flex items-center">Events Won</div>
                   <div className="col-span-4 p-3 bg-slate-900/50 text-center text-white font-black h-full flex items-center justify-center">{(currentStats.winTrades).toFixed(0)}</div>
                   <div className="col-span-3 p-3 bg-slate-900/50 text-right text-slate-500 pr-4 font-black h-full flex items-center justify-end">-</div>
                </div>
                <div className="grid grid-cols-12 h-11 items-center">
                   <div className="col-span-5 p-3 bg-slate-800/20 text-slate-400 h-full flex items-center">Events Lost</div>
                   <div className="col-span-4 p-3 bg-slate-900/50 text-center text-white font-black h-full flex items-center justify-center">{(currentStats.lossTrades).toFixed(0)}</div>
                   <div className="col-span-3 p-3 bg-slate-900/50 text-right text-slate-500 pr-4 font-black h-full flex items-center justify-end">-</div>
                </div>
             </div>
          </div>
        ...
        <div className="col-span-12 lg:col-span-3 space-y-4">
          ...
          <div className="bg-[#252b39] border border-slate-700 rounded overflow-hidden shadow-xl">
             ...
             <div className="p-0 text-[10px] font-bold uppercase">
                ...
                <div className="flex justify-between items-center p-2.5 border-b border-slate-800/40">
                   <span className="text-slate-400 font-black tracking-tighter">Sessions Required</span>
                   <span className="text-white font-black">{sessionsRequired}</span>
                </div>
                <div className="flex justify-between items-center p-2.5 border-b border-slate-800/40">
                   <span className="text-slate-400 font-black">Account Gain</span>
                   <span className="text-white font-black">{(currentStats.accountGainPercent > 0 ? currentStats.accountGainPercent : 0).toFixed(2)}%</span>
                </div>
